...
const EditPasswordModal: React.FC<EditPasswordModalProps> = ({ customer }) => {
  ...
  const submit = handleSubmit(async (data) => {
    ...
    if (data.old_password === data.new_password) {
      setSubmitting(false)
      setError("La contraseña nueva debe ser distinta a la antigua.")
      return
    }
    ...
    if (!passwordMatches) {
      setError("La contraseña antigua no coincide con nuestro registro.")
      setSubmitting(false)
      return
    }
    ...
    update(
      { id: customer.id, password: data.new_password },
      {
        ...
        onError: () => {
          setSubmitting(false)
          setError("No se puede actualizar la contraseña, inténtalo más tarde.")
        },
      }
    )
  })
